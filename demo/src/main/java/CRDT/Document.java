...
public class Document {
    ...
    private Map<PositionID, CRDT.BlockNode> blockSequence;
    ...
    public Document(String authorId, String documentId, String title) {
        ...
        CRDT.BlockNode firstBlock    = new CRDT.BlockNode(firstBlockId);
    ...
    public Operation addParagraph(int paragraphIndex, String initialText) {
        ...
        CRDT.BlockNode newBlock   = new CRDT.BlockNode(newBlockId);
    ...
    public Operation removeParagraph(PositionID blockId) {
        ...
        CRDT.BlockNode block = blockSequence.get(blockId);
    ...
    public Operation breakParagraph(PositionID blockId, int splitIndex) {
        ...
        List<CRDT.CharNode> visibleChars = getVisibleChars(block);
        ...
        for (int i = splitIndex; i < visibleChars.size(); i++) {
            CRDT.CharNode ch = visibleChars.get(i);
    ...
    public Operation eraseCharacter(PositionID blockId, PositionID charId) {
        ...
        CRDT.CharNode target = block.getNodeMap().get(charId);
    ...
    public void receiveEdit(Operation incoming) {
        ...
        switch (incoming.getType()) {
            ...
            case INSERT_CHAR:
                CRDT.BlockNode b1 = blockSequence.get(incoming.getTargetBlockId());
            ...
            case DELETE_CHAR:
                CRDT.BlockNode b2 = blockSequence.get(incoming.getTargetBlockId());
                if (b2 != null)
            ...
            case INSERT_BLOCK:
                if (!blockSequence.containsKey(incoming.getTargetBlockId())) {
                    CRDT.BlockNode fresh = new CRDT.BlockNode(incoming.getTargetBlockId());
            ...
            case DELETE_BLOCK:
                CRDT.BlockNode b3 = blockSequence.get(incoming.getTargetBlockId());
                if (b3 != null) {
                    b3.markDeleted();
    ...
    // ─────────────────────────────────────────
    private List<CRDT.CharNode> getVisibleChars(CRDT.BlockNode block) {
        List<CRDT.CharNode> visible = new ArrayList<>();
        collectVisible(block, visible);
        return visible;
    }

    private void collectVisible(CRDT.BlockNode block, List<CRDT.CharNode> result) {
        // BlockNode already does DFS traversal in toString()
        // We replicate that here but collect nodes instead of chars
        for (CRDT.CharNode root : block.getRoots()) {
            dfsCollect(root, result);
        }
    }

    private void dfsCollect(CRDT.CharNode node, List<CRDT.CharNode> result) {
        if (!node.isDeleted()) result.add(node);
        for (CRDT.CharNode child : node.getChildren()) {
    ...
    private int getCharIndex(PositionID blockId, PositionID charId) {
        ...
        List<CRDT.CharNode> visible = getVisibleChars(block);
    ...
    // Getters
    // ─────────────────────────────────────────
    public Map<PositionID, CRDT.BlockNode> getBlockSequence() {
